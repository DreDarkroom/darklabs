...
// --- Pro UI Components ---
const BulkActionsPanel = ({ selectedCount }) => (
    <div className="bg-neutral-900 border border-neutral-800 p-4 rounded mb-6 flex items-center justify-between">
        <div className="flex items-center gap-4">
            <span className="text-sm text-neutral-400 uppercase tracking-widest">{selectedCount} Tracks Selected</span>
            <button className="px-3 py-1 bg-neutral-800 hover:bg-neutral-700 text-xs text-white rounded transition-colors" disabled={selectedCount === 0}>
                Normalise LUFS
            </button>
            <button className="px-3 py-1 bg-neutral-800 hover:bg-neutral-700 text-xs text-white rounded transition-colors" disabled={selectedCount === 0}>
                Auto-Fade
        ...
        <div className="flex gap-2">
            <button className="px-4 py-2 bg-dre-red hover:bg-dre-red-soft text-xs font-bold text-white rounded transition-colors uppercase tracking-widest" disabled={selectedCount === 0}>
                Export to Ableton
            </button>
        </div>
    </div>
...
function EarshotApp() {
    ...
    const [tracks, setTracks] = useState(MOCK_TRACKS.map(t => ({...t, rating: null, selected: false})));
    ...
    const toggleSelection = (id) => {
        setTracks(tracks.map(t => t.id === id ? { ...t, selected: !t.selected } : t));
    };
    ...
    if (!isAuthenticated) {
        return (
            <div className="flex items-center justify-center min-h-screen bg-neutral-950">
                <div className="bg-neutral-900 border border-neutral-800 p-8 rounded shadow-2xl max-w-sm w-full text-center">
                    <h1 className="text-3xl font-bold mb-2 tracking-wider text-dre-red-soft">Earshot</h1>
                    <p className="text-neutral-500 mb-8 tracking-widest text-xs uppercase">Pro Authentication</p>
                    <button
                        onClick={() => setIsAuthenticated(true)}
                        className="w-full px-6 py-3 bg-neutral-800 hover:bg-neutral-700 border border-neutral-700 hover:border-neutral-500 text-white font-bold text-sm tracking-wide rounded transition-all mb-4"
                    >
                        Sign In via SSO (Mock)
                    </button>
                    <p className="text-[10px] text-neutral-600">Requires a valid team license.</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="flex items-center justify-center min-h-screen">
                <div className="text-center">
                    ...
                    <p className="text-neutral-400 mb-8 tracking-widest text-sm">PRO EDITION</p>
                    <button
                        onClick={handleStart}
    ...
    return (
        <div className="max-w-5xl mx-auto p-4 md:p-8">
            <header className="flex justify-between items-end mb-8 border-b border-neutral-800 pb-4">
                <div>
                    ...
                    <div className="flex items-center gap-3 mt-1">
                        <p className="text-neutral-500 text-sm tracking-widest uppercase">Pro Edition</p>
                        <span className="px-2 py-0.5 bg-neutral-800 border border-neutral-700 text-neutral-400 text-[10px] uppercase rounded tracking-wider">Audio Team Alpha</span>
                    </div>
                </div>
                <div className="flex gap-4 items-center">
                    <button onClick={() => setShowTutorial(!showTutorial)} className="text-xs text-neutral-500 hover:text-white tracking-widest uppercase underline">
            ...
            <BulkActionsPanel selectedCount={tracks.filter(t => t.selected).length} />

            <div className="grid gap-4">
                {tracks.map(track => (
                    <div
                        key={track.id}
                        className={`flex items-center justify-between p-4 bg-neutral-900 border-l-4 rounded shadow-sm transition-colors ${track.color} ${activeTrack === track.id ? 'ring-1 ring-dre-red-soft bg-neutral-800' : ''} ${track.selected ? 'bg-neutral-800/80' : ''}`}
                    >
                        <div className="pl-2 pr-4 flex items-center">
                            <input
                                type="checkbox"
                                className="w-4 h-4 accent-dre-red bg-neutral-800 border-neutral-700 rounded cursor-pointer"
