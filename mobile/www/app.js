...
// These all no-op harmlessly in a plain desktop browser (GitHub Pages), and
// come alive when the same www/ is wrapped in the Capacitor Android app.

function isNative() {
  return !!(window.Capacitor && window.Capacitor.isNativePlatform && window.Capacitor.isNativePlatform());
}

// One quiet tick per note — the kind of tactile confirmation a hardware pad
// controller gives you for free. Uses the native Haptics plugin inside the
// Android app, falls back to the Vibration API on Android Chrome, and is
// just a no-op everywhere else (iOS Safari has neither).
function hapticTick() {
  try {
  ...
  } catch (_) { /* plugin not present in this build — ignore */ }
  if (navigator.vibrate) navigator.vibrate(8);
}

// Keep the screen awake for the length of a play session — nobody wants the
// display to dim mid-riff. Released as soon as the tab/app is backgrounded.
async function acquireWakeLock() {
  try {
    if ("wakeLock" in navigator) {
      wakeLock = await navigator.wakeLock.request("screen");
    }
  } catch (_) { /* not supported, or denied — fine, just no wake lock */ }
}
document.addEventListener("visibilitychange", () => {
  const panel = document.getElementById("panel");
  const started = panel && panel.hidden === false;
...
function setUpNativeChrome() {
  ...
  try {
    ...
    if (StatusBar) {
      ...
      StatusBar.setStyle({ style: "DARK" }); // dark background -> light icons
    }
  } catch (_) { /* status bar plugin not present — ignore */ }
}
...
(function setRepoLink() {
  ...
  const FALLBACK = "https://github.com/DreDarkroom/darklabs";
  if (host.endsWith("github.io")) {
    const user = host.split(".")[0];
    const repo = location.pathname.split("/").filter(Boolean)[0];
    a.href = user && repo ? `https://github.com/${user}/${repo}` : FALLBACK;
  } else {
    // Inside the native app (or any other host) there's no page URL to
    // infer the repo from, so point straight at it.
    a.href = FALLBACK;
  }
})();
